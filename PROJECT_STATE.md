...
1. Приёмка Gateway на реальном дубле NSP (путь «использовать существующую», сверка остатка «Т кредитка»).
2. Сделать репозиторий Private на GitHub.
3. Проверить v51 (поле «Магазин»), затем Фаза 2.
