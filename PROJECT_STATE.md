...
- **Главный риск — потеря файлов.** 2026-07-25 обнаружено: из `api/` пропал весь backend-код, сервис жил на образе от 21.07. Восстановлено из контейнера. Git поднять в приоритете, не дожидаясь конца волны.
...
1. Проверить v51 на смартфоне: поле «Магазин» (популярные / поиск / «Создать новый»).
2. Git: локальный репозиторий + усиленный `.gitignore`.
3. Приёмка шлюза: импорт NSP → решения дубля → сверка счетов.
